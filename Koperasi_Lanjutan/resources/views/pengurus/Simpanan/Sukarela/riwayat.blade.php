...
@section('content')
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <form action="{{ route('pengurus.simpanan.sukarela.riwayat') }}" method="GET">
            <div
                class="bg-gradient-to-br rounded-2xl shadow-lg flex flex-col items-center justify-center aspect-square w-56 hover:shadow-xl transition">
            ...
            <div class="mt-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-5 rounded-2xl shadow-sm">
                    {{-- Filter Anggota (ID) --}}
                    <div>
                        <label for="id" class="block text-sm font-semibold text-gray-700 mb-1">ID Anggota
                            (Opsional)</label>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($riwayat as $index => $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-sn text-gray-700">{{ $item->user->nama ?? 'Anggota' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-4 text-center text-gray-500 italic">Tidak ada data riwayat
                                simpanan</td>
                        </tr>
                    @endforelse
                </tbody>
