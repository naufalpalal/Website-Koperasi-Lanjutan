...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 py-10">
        ...
        <div class="bg-white shadow-lg rounded-xl p-6 w-full max-w-xl mx-auto mb-10">
                ...
                <form action="{{ route('tabungan.index') }}" method="GET" class="space-y-4">
                    <div>
                        ...
                        <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}"
                            min="{{ date('Y-m-d') }}" pattern="\d{4}-\d{2}-\d{2}"
                    ...
                    <div>
                        ...
                        <input type="number" id="nilai" name="nilai" value="{{ old('nilai') }}" min="100" step="100"
                            placeholder="Minimal 100" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm px-3 py-2"
                            required>
                    </div>
            ...
            @else
                {{-- Tahap 2 --}}
                <form action="{{ route('user.simpanan.tabungan.store') }}" method="POST" enctype="multipart/form-data"
                    class="space-y-4">
                    @csrf
                    ...
                    <div class="flex flex-col items-center space-y-3">
                        ...
                        {{-- TOMBOL DOWNLOAD QR --}}
                        <a href="{{ asset('assets/moh.naufal16_qr.png') }}" download="qr_pembayaran.png"
                            class="bg-gray-700 hover:bg-gray-800 text-white px-4 py-2 rounded-lg shadow text-sm">
                            Download QR Code
                        </a>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" id="bukti_transfer" name="bukti_transfer" accept="image/png, image/jpeg, image/jpg"
                            ...
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    @elseif ($item->status == 'diterima')
                                        <span
                                            class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-xs font-semibold">Diterima</span>
                                    @elseif ($item->status == 'dipotong')
                                        <span
                                            class="bg-red-100 text-red-800 px-3 py-1 rounded-full text-xs font-semibold">Dipotong</span>
                                    @elseif ($item->status == 'ditolak')
                                        <span
                                            class="bg-gray-100 text-gray-800 px-3 py-1 rounded-full text-xs font-semibold">Ditolak</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
